...
- **Quizlet Privacy Policy:** normal public-browser access succeeded on the first attempt, within the five-minute limit. The rendered policy, California disclosure table, jurisdictional rights, and final Contact Us section were inspected. No login, CAPTCHA solving, or private account extraction.
...
The five final JSON files were mechanically copied from normalized captures, not manually composed policy text. Temporary acquisition responses and intermediate files are not included in the repository.

Run `node --test test/snapshots.test.mjs` to check the published snapshots' catalog IDs/URLs, capture methods, dates, and policy-content validation. This does not reproduce the original capture or prove equivalence to today's upstream pages. `npm run capture:sources` attempts a fresh network capture; pages requiring browser rendering may need a new manual public capture.
